...
class labelme2coco(object):
    def __init__(self, labelme_json=[], save_json_path="./coco.json", data_set_name = "dataset_BRUMAS_v1"):
        """
        ...
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, "r") as fp:
                ...
                for shapes in data["shapes"]:
                    ...
                    points = np.array(shapes["points"]).astype(float).tolist()
                    self.annotations.append(self.annotation(points, label, num))
                    self.annID += 1

        with open (f"./{self.data_set_name}/classesNumber.txt", "r") as classesNames:
            classes = classesNames.read().splitlines()
        
        for i in range(len(classes)):
            self.all_class_labels.append([' '.join(classes[i].split(":")[-1].split(" ")[1:])])
        self.label.sort()
        for label in self.all_class_labels:
...
if __name__ == "__main__":
    ...
    labelme2coco(labelme_json_test, save_test, data_set_name)
    labelme2coco(labelme_json_train, save_train, data_set_name)
    print(args.labelme_images_train)
